...
Mac 版登录成功后如果出现“系统提示”弹窗，自动化流程会先尝试点击标准 `确定` 按钮；如果客户端没有暴露标准按钮，还会通过截图识别蓝色 `确定` 按钮位置并点击。点掉弹窗后，程序会再点击左上方黑色输入区并按一次 `ESC`，让绿色输入箭头出现，之后才进入可查询状态。

如果黑屏提示 `#用户异常,请重新登录!!!`，系统会把会话标记为 `session_expired`。此时再次调用登录接口，Mac 自动化会先关闭并重新打开 `iEterm Mac版`，再重新进入登录流程。

### `POST /session/close`

关闭电脑上的 `iEterm Mac版`。Mac 版关闭时如果弹出“关闭软件将会造成工作中断，是否确认退出软件？”之类的系统提示，自动化流程会点击 `是`。

### `GET /session/screenshot`

返回当前电脑屏幕截图，用来在手机页面确认 `iEterm` 当前状态。Mac 上建议把 `iEterm Mac版` 窗口撑满并保持在最前面；如果截图为空或黑屏，需要给运行服务的程序开启 macOS `屏幕录制` 权限。
...
页面支持保存 Token、选择线路、登录、关闭 iEterm、国际票价表单查询和原始查询指令输入。每次操作结束后，页面会自动刷新当前 `iEterm` 截图，方便判断黑屏是否报错以及下一步该输入什么。手机页面会把发送过的原始指令保存在本机浏览器历史中，之后可以点击历史指令直接再次发送。
...
实际黑屏里常用的基础查票指令可以通过“原始查询指令”发送，例如：

```text
AVH/CANHRE30JUN/ADD
```

系统已允许 `AVH` 这类查询指令，并会尝试从返回中解析航班号、起飞时间、落地时间和主要舱位余位。

Mac 版发送指令后会自动按 `Enter` 键，也就是键盘上的 `fn + enter`，用于触发 iEterm 查询。
...
执行手机或外部调用方输入的原始 iEterm 指令。该接口不再限制指令类型，只会拒绝空指令。
...
### `POST /query/fare-calculation`

用于票价计算弹窗。程序会点击当前黑屏页面里的 `票价计算`，等待弹窗出现后点击 `复制内容`，并把复制到剪贴板的弹窗原文返回给手机页面。
...
这个接口只点击 `登 录` 按钮，不读取、不保存你的密码。登录成功后如果出现“系统提示”弹窗，会继续点击 `确定`；如果按钮是 iEterm 内部画出来的非标准按钮，程序会截图识别蓝色按钮后再点击。第一次建议先人工确认用户名、密码、认证方式、服务器和端口都正确，并让 `iEterm Mac版` 窗口保持在最前，避免其他窗口遮挡确认弹窗。
