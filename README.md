...
Mac 版登录成功后如果出现“系统提示”弹窗，自动化流程会先尝试点击标准 `确定` 按钮；如果客户端没有暴露标准按钮，还会通过截图识别蓝色 `确定` 按钮位置并点击，点掉后才进入可查询状态。
...
页面支持保存 Token、选择线路、登录、关闭 iEterm、国际票价表单查询和原始查询指令输入。每次操作结束后，页面会自动刷新当前 `iEterm` 截图，方便判断黑屏是否报错以及下一步该输入什么。
